# XiaoZhi Mobile App v0.6.2

Android 小智手机助手：本地离线唤醒、本地中文语音识别、透明桌面语音悬浮层、连续会话、任意已安装 App 发现/启动诊断、地图导航/附近搜索，以及受严格白名单约束的 AI 安全工具调用。

## v0.6.2 重点

- **自定义唤醒自动同步**：启动离线唤醒和“保存全部设置”都会把编辑框中的唤醒短语同步到正在运行的 KWS；页面显示“当前实际 KWS 唤醒短语”。
- **执行完成语音确认**：本地手机指令与 AI 安全工具执行成功后会播报“已打开… / 音量已… / 导航已经打开”等短句，TTS 完成后 120ms 重新监听。
...
本仓库 v0.6.2 是**直接安装版**，为了实现尽可能完整的 App 发现使用 `QUERY_ALL_PACKAGES`。未来如果发布 Google Play，需要单独做商店合规版本并重新评估该权限。

## 构建

推送到 GitHub 后，Actions 会下载 KWS + Paraformer 本地模型并生成：

`XiaoZhi-Mobile-v0.6.2-debug.apk`
